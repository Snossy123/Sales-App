...
export function EmployeeAttachmentsSection({
  employeeId,
  mode,
  pendingFiles = [],
  onPendingChange,
  canManage = true,
}: EmployeeAttachmentsSectionProps) {
  ...
  const mediaQuery = useQuery({
    queryKey: ['employee-media', employeeId],
    queryFn: async () => {
      const { data } = await api.get<{ data: MediaFile[] }>(`/employees/${employeeId}/media`)
      return data.data
    },
    enabled: mode === 'view' && Boolean(employeeId),
  })
  ...
  const uploadMutation = useMutation({
    mutationFn: async (payload: { file: File; description?: string }) => {
      ...
      const { data } = await api.post<MediaFile>(`/employees/${employeeId}/media`, formData)
      return data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['employee-media', employeeId] })
      setUploadError('')
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (mediaId: number) => {
      await api.delete(`/employees/${employeeId}/media/${mediaId}`)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['employee-media', employeeId] })
    },
  ...
  return (
    <section className="rounded-lg border border-outline-variant bg-surface-container-lowest p-md sm:col-span-2">
...
export async function uploadEmployeeAttachments(
  employeeId: number,
  pendingFiles: PendingAttachment[],
): Promise<void> {
  for (const item of pendingFiles) {
    ...
    await api.post(`/employees/${employeeId}/media`, formData)
  }
}
